...
app.get('/scrape', async (req, res) => {
    ...
    try {
        ...
        await page.waitForTimeout(5000);  // wait for JS to finish loading
        ...
        let listings = await page.evaluate(() => {
            ...
            selectors.forEach(selector => {
                document.querySelectorAll(selector).forEach(el => {
                    ...
                    const priceTag = el.innerText.match(/\$\d{1,3}(,\d{3})*(\.\d{2})?/);
                    const linkTag = el.querySelector('a');
                    if (titleTag && priceTag && linkTag) {
                        let price = priceTag[0].replace('$', '').replace(/,/g, '');
                        if (!isNaN(price)) {
                            items.push({
